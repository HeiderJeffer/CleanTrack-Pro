...
with materials_tab:
    ...
    st.caption(
        "Automatic total consumption based on saved working days."
    )
    ...
    st.info(
        f"Based on **{total_working_days} working days** "
        f"recorded in CleanTrack Pro."
    )
    ...
    with col1:
        ...
        st.metric(
            ...
            f"{total_floor_sanitary:.2f} bottles"
        )
    ...
    with col2:
        ...
        st.metric(
            "🪟 Glass Cleaner",
            f"{total_glass_cleaner:.2f} bottles"
        )

        st.metric(
            "🚿 Bathroom Barrier & Floor Cleaner",
            f"{total_bathroom_cleaner:.2f} bottles"
        )
    ...
    st.write(
        "💧 Water: **12 liters / working day**"
    )

    st.write(
        "🧴 Floor & Sanitary Facilities Cleaner: "
        "**1 bottle / 5 working days**"
    )
    ...
    st.download_button(
        ...
        data=csv_data,
        file_name="cleantrack_total_consumption.csv",
        mime="text/csv",
        use_container_width=True
    )
